...
function getPickerMessageOrigin() {
    return new URL(getPickerBaseUrl(), window.location.href).origin
}
...
})
{
    ...
    let PICKER_ORIGIN = getPickerBaseUrl()
...
function connectToPickerWindow(pickerOrigin: string, fhirVersion: FhirVersion, win: Window, onClose: (data: any) => void)
{
    ...
    function normalizeSelection(data: any) {
        if (!data) {
            return data
        }
    ...
    function onMessage(e: MessageEvent) {
        ...
        if (e.origin === pickerOrigin) {
            ...
            if (e.data.type === 'result' || e.data.type === 'close' || e.data.type === 'setPatient') {
                window.removeEventListener('message', onMessage);
                // The picker may send the selected value under different keys.
                // Historically some pickers use `data`, others `payload` — accept both.
                const selectionRaw = e.data.data ?? e.data.payload ?? e.data;
                onClose(normalizeSelection(selectionRaw));
            }
        }
    }
