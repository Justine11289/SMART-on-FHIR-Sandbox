...
FHIR_VERSIONS.forEach(([ver, server]) => {
    ...
    describe("FHIR Proxy " + ver, () => {
        ...
        it ("Rewrites localhost pagination links", async () => {
            ...
            server.mock("/Patient", {
                ...
                body: {
                    ...
                    link: [{
                        relation: "next",
                        url: "http://localhost:8080/hapi-fhir-jpaserver/fhir?_getpages=abc&_count=200"
                    }],
                    entry: []
                },
                headers: { "content-type": "application/json" }
            ...
            const body = await fetch(LAUNCHER.baseUrl + "/v/" + ver + "/fhir/Patient", {
                headers: { authorization: `Bearer ${ACCESS_TOKEN}` }
            }).then(r => r.json());

            const next = body.link.find((l: any) => l.relation == "next")
            expect(next, "No next link found").to.exist;
            expect(next.url).to.equal(LAUNCHER.baseUrl + "/v/" + ver + "/fhir?_getpages=abc&_count=200")
        });
