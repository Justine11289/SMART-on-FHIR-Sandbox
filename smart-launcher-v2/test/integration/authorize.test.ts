    ...
    describe(`${fhirVersion} FHIR Server`, () => {
        ["GET", "POST"].forEach(method => {
            describe(`authorize endpoint using ${method} request`, () => {
                ...
                it (`accepts /index.html redirect_uri for a whitelisted static app path`, async () => {
                    const res = await launch({
                        launchParams  : {
                            launch_type: "provider-ehr",
                            redirect_uris: "https://example.com/Clinical-Decision-Calculator-Platform/"
                        },
                        requestOptions: { method },
                        redirect_uri  : "https://example.com/Clinical-Decision-Calculator-Platform/index.html",
                        response_type : "code",
                        fhirVersion
                    });
                    expect(res.status).to.equal(302)
                ...
                describe("patient-portal launch", () => {
                    ...
                    it (`continues if auto-selecting the first encounter fails`, async () => {
                        const server = FHIR_VERSIONS.find(arr => arr[0] === fhirVersion)![1];
                        ...
                        const redirectUrl = new URL(res.headers.get("location") || "")
                        const code = parseResponseCode(redirectUrl.searchParams.get("code") || "")
                        expect(code.context.encounter).to.not.exist
                    });
                })
