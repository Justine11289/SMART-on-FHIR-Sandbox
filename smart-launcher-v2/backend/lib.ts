...
export function rewriteFhirUrl(value: string, upstreamBaseUrl: string, publicBaseUrl: string) {
    let rewritten = value.replaceAll(upstreamBaseUrl, publicBaseUrl);
    ...
    try {
        ...
        if (upstreamPath) {
            rewritePaths.add(upstreamPath);
        }

        // HAPI FHIR in this sandbox emits pagination links under its internal
        // app path, even when the upstream server is mocked in tests.
        rewritePaths.add("/hapi-fhir-jpaserver/fhir");

        for (const path of rewritePaths) {
            const localPattern = new RegExp(
                `https?:\\/\\/(?:localhost|127\\.0\\.0\\.1)(?::\\d+)?${escapeRegExp(path)}`,
                "g"
            );

            rewritten = rewritten.replace(localPattern, publicBaseUrl);
        }
    } catch {
        // Fall back to the direct string replacement above when the upstream
