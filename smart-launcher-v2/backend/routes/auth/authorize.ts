...
import { putKeycloakAuthState } from "../../lib/KeycloakAuthStateCache"
...
export default class AuthorizeHandler {
    ...
    public renderKeycloakLogin(): void {
        const callbackUrl = new URL(this.request.baseUrl + "/keycloak-callback", this.baseUrl)
        const state = crypto.randomUUID()

        putKeycloakAuthState(state, {
            returnTo: this.request.originalUrl,
            clientId: this.params.client_id,
            redirectUri: callbackUrl.href,
            createdAt: Date.now()
        })

        const kcAuth = new URL(KEYCLOAK_ISSUER.replace(/\/+$/, "") + "/protocol/openid-connect/auth")
        kcAuth.searchParams.set("response_type", "code")
        kcAuth.searchParams.set("client_id", this.params.client_id)
        kcAuth.searchParams.set("redirect_uri", callbackUrl.href)
        kcAuth.searchParams.set("scope", "openid")
        kcAuth.searchParams.set("state", state)
        return this.response.redirect(kcAuth.href)
    }
    ...
    public renderEncounterPicker(): void {
        ...
        if (this.launchOptions.encounter === "AUTO") {
            this.getFirstEncounterId().then(
                id => {
                    this.launchOptions.encounter = id || "NONE"
                    this.authorize()
                }
            )
            .catch(() => {
    ...
    private normalizeRedirectUri(url: URL) {
        const normalized = new URL(url.href)
        normalized.pathname = normalized.pathname.replace(/\/index\.html$/, "/").replace(/\/+$/, "/")
    ...
    public authorize()
    {
        ...
        if (launchOptions.redirect_uris) {
            ...
            if (!urls.some(u => {
                ...
                return this.normalizeRedirectUri(url).startsWith(this.normalizeRedirectUri(u));
            })) {
                throw new InvalidRequestError(`Invalid redirect_uri`)
            }
        }

        // Route through Keycloak after patient/encounter selection but before
        // the launcher completes authorization.
        if (KEYCLOAK_ENABLED && KEYCLOAK_ISSUER && params.prompt !== "none" && params.login_success !== "1") {
            return this.renderKeycloakLogin();
        }
