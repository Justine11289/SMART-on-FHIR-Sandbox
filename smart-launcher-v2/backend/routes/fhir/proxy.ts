...
import { getRequestBaseURL, getFhirServerBaseUrl, rewriteFhirUrl, validateToken } from "../../lib"
...
export default async function proxy(req: Request, res: Response) {
    ...
    const publicBaseUrl = `${getRequestBaseURL(req)}${req.baseUrl}`;

    ["content-type", 'etag', 'location'].forEach(name => {
        if (response.headers.has(name)) {
            const headerValue = response.headers.get(name) + "";
            res.set(name, name === "location" ? rewriteFhirUrl(headerValue, fhirServer, publicBaseUrl) : headerValue)
        }
    })

    if (!isBinary) {
        let body = await response.text()
        body = rewriteFhirUrl(body, fhirServer, publicBaseUrl);
        res.end(body);
    } else {
        // @ts-ignore
